...
document.getElementById('download-svg').addEventListener('click', function () {
    ...
    link.download = 'mise_en_oeuvre_et_resultats.svg';
    link.click();
});
...
document.getElementById('download-png').addEventListener('click', function () {
    html2canvas(document.getElementById('content-to-capture'), {
        ...
        canvas.toBlob(function (blob) {
            var link = document.createElement('a');
            link.href = URL.createObjectURL(blob);
            link.download = 'mise_en_oeuvre_et_resultats.png';
            link.click();
        });
    });
});
        ...
        if (index < containers.length) {
            ...
            gifshot.createGIF({
                ...
                if (!obj.error) {
                    var link = document.createElement('a');
                    link.href = obj.image;
                    link.download = 'mise_en_oeuvre_et_resultats.gif';
                    link.click();
                }
            });
        }
...
document.addEventListener('keydown', function(event) {
    if (event.key === 'ArrowRight') {
        window.location.href = '/scrap_result';  // Replace with actual next page URL
    }
    if (event.key === 'ArrowLeft') {
        window.location.href = '/data_ai';  // Replace with actual previous page URL
    }
});